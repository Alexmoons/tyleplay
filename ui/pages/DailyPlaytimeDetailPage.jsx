...
function buildMostPlayedPeriodGames(sessionEntries, games, library) {
  ...
  const now = new Date();
  const periodStart = localMidnightTimestamp(Math.floor(now.getTime() / 1000));
...
const DAILY_PLAYTIME_DETAIL_PAGE_SIZE = 15;
    ...
    <div ref={rootRef} className={`sort-select${isOpen ? " is-open" : ""}`}>
      ...
      {isOpen ? (
        <div className="sort-select-panel">
          <div className="sort-select-option-list" role="listbox" aria-label="Sort daily playtime detail">
            {options.map((option) => (
              <button
                key={option.value}
                type="button"
...
export default function DailyPlaytimeDetailPage({
  ...
  const todayRawGames = useMemo(() => {
    return buildMostPlayedPeriodGames(sessionEntries, statsSnapshot?.games, library);
  }, [sessionEntries, statsSnapshot, library]);

  const sortedGames = useMemo(() => {
    return [...todayRawGames].sort((left, right) => {
      switch (sortBy) {
        case "name-asc":
  ...
  }, [todayRawGames, sortBy]);

  const totalTodaySeconds = useMemo(
    () => todayRawGames.reduce((sum, g) => sum + Math.max(0, Number(g.total_seconds || 0)), 0),
    [todayRawGames]
  );

  const totalPages = Math.max(1, Math.ceil(sortedGames.length / DAILY_PLAYTIME_DETAIL_PAGE_SIZE));
  const currentPage = Math.min(page, totalPages);
  const visibleGames = sortedGames.slice((currentPage - 1) * DAILY_PLAYTIME_DETAIL_PAGE_SIZE, currentPage * DAILY_PLAYTIME_DETAIL_PAGE_SIZE);
  ...
  return (
    <div className="playtime-detail-page daily-playtime-detail-page">
      <header className="page-header page-header-library playtime-detail-header">
        <div className="page-heading">
          <h1>Today's Playtime</h1>
          <p className="text-xs text-gray-400 mt-1 font-medium">{todayDateLabel}</p>
        </div>
        ...
        <div className="daily-playtime-view-table-wrap">
          <button
            ...
            onClick={onOpenTable}
            title="View Daily Playtime History Table"
          >
      ...
      <section className="stats-section">
        <div className="playtime-detail-topbar">
          ...
          <div className="playtime-detail-topbar-meta">
            <span>{todayRawGames.length} titles</span>
            <span aria-hidden="true" className="playtime-detail-topbar-separator">•</span>
            <span>Total playtime today: {formatDurationLong(totalTodaySeconds)}</span>
          </div>
        </div>
        ...
        <article className="stats-panel playtime-detail-panel">
          <div className={`stats-topgames-list playtime-detail-list ${topGameArtwork === "capsule" ? "is-capsule" : "is-poster"}`}>
            {isPageLoading ? (
              Array.from({ length: DAILY_PLAYTIME_DETAIL_PAGE_SIZE }, (_, idx) => (
              ...
              visibleGames.map((game) => {
                ...
                const share = totalTodaySeconds > 0 ? (totalSeconds / totalTodaySeconds) * 100 : 0;
          ...
          {!sortedGames.length && !isPageLoading ? (
            <div className="stats-empty">
              <strong>No games played today.</strong>
              <span>Play a game today to populate this activity list.</span>
            </div>
          ) : null}
        </article>
      </section>
